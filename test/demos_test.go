...
func TestAll(t *testing.T) {
	...
	files, _ := os.ReadDir(demo_path)
	for _, file := range files {
		fmt.Println("执行", file.Name())

		data, _ := os.ReadFile(path.Join(demo_path, file.Name()))
...
func TestDemo1ByStep(t *testing.T) {
	...
	data, _ := os.ReadFile(demo_path + "/1简单流程.json")
...
func TestFlowRunner(t *testing.T) {
	...
	data, _ := os.ReadFile(demo_path + "/4执行脚本.json")
...
func TestScriptCmd(t *testing.T) {
	...
	data, _ := os.ReadFile(demo_path + "/5执行命令.json")
